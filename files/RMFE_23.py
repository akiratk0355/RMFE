...
_sage_const_51 = Integer(51)  # !/usr/bin/env sage
...
H = GF(_sage_const_2 ** _sage_const_51, modulus="primitive", names=('c',))
(c,) = H._first_ngens(1)
h = H.modulus()

F = GF(_sage_const_2 ** _sage_const_3, names=('a',))
(a,) = F._first_ngens(1)
f = F.modulus()
...
def psi_RMFE23(w, k):
    ...
    for i in range(len(k)):
        if k[i] > _sage_const_18:
            raise Exception(
                "every coordinate on second input of psi_RMFE need to be at most 18")
    B = [a**i for i in range(_sage_const_3)]
    data = FFTpreproc(_sage_const_3, B)
    ...
    for j in range(len(w)):
        ...
        hredi = listsum(m[_sage_const_0:_sage_const_8], [
                        _sage_const_0]+m[_sage_const_8:_sage_const_15])
