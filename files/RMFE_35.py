...
_sage_const_63 = Integer(63)  # !/usr/bin/env sage
...
H = GF(_sage_const_2 ** _sage_const_325, modulus="primitive", names=('c',))
(c,) = H._first_ngens(1)
h = H.modulus()

F = GF(_sage_const_2 ** _sage_const_5, names=('a',))
(a,) = F._first_ngens(1)
f = F.modulus()
...
def psi_RMFE35(w, k):
    ...
    for i in range(len(k)):
        if k[i] > _sage_const_99:
            raise Exception(
                "every coordinate on second input of psi_RMFE24 needs to be at most 99")
    B = [a**i for i in range(_sage_const_5)]
    data = FFTpreproc(_sage_const_5, B)
    ...
    for j in range(len(w)):
        ...
        hred = listsum(m[_sage_const_0:_sage_const_32], [
                       _sage_const_0]+m[_sage_const_32:_sage_const_63])
